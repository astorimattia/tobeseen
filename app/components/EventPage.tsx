...
interface Event {
  ...
  images: string[];
  analogImages?: string[];
  mediumUrl?: string; // Optional field for Medium article URL (legacy single story)
  mediumTitle?: string; // Optional title for the Medium article (legacy)
  stories?: StoryLink[]; // New: support multiple stories
  vimeoId?: string;
  videoUrl?: string;
}
      ...
      >
        ...
        <article className="space-y-8 mt-4 md:mt-8">
          <div className="space-y-6">
            ...
            {/* Story Links - Support both legacy single link and new multiple stories */}
            {(event.mediumUrl || event.stories) && (
              <div className="mt-8 space-y-4">
                {/* Legacy single story link */}
                {event.mediumUrl && !event.stories && (
                  <Link
                ...
                {/* New: Multiple stories support */}
                {event.stories && event.stories.map((storyLink, index) => {
                  const LinkComponent = storyLink.isExternal ? 'a' : Link;
                  const linkProps = storyLink.isExternal 
                    ? { href: storyLink.url, target: "_blank", rel: "noopener noreferrer" }
                    : { href: storyLink.url };
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">
                            {storyLink.isExternal ? 'Read on Substack' : 'Read Full Story'}
                          </p>
